...
class LoginRegisterController extends Controller
{
    ...
    public function loginConfirmForm($token)
    {
        ...
        if(empty($otp))
        {
            return redirect()->route('auth.customer.login-register-form')->withErrors(['id' => 'آدرس وارد شده نا معتبر می باشد']);
        }
    ...
    public function loginConfirm(LoginRegisterRequest $request,$token)
    {
        ...
        if (empty($otp))
        {
            return redirect()->route('auth.customer.login-register-form',$token)->withErrors(['id'=>'آدرس وارد شده نا معتبر است']);
        }
        ...
        {
            return redirect()->route('auth.customer.login-confirm-form')->withErrors(['otp'=>'رمز وارد شده صحیح نمی باشد']);
        }
    ...
    public function loginResendOtp($token)
    {
      ...
      if(empty($otp))
      {
          return redirect()->route('auth.customer.login-register-form',$token)->withErrors(['id' => 'آدرس وارد شده صحیح نمیباشد']);
      }
