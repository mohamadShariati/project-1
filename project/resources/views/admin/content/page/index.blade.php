                ...
                <table class="table table-striped table-hover">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{$page->title}}</td>
                            <td>
                                <input type="checkbox" onchange="changeStatus({{$page->id}})" data-url="{{route('admin.content.page.status',$page->id)}}" id="{{$page->id}}" value="" @if ($page->status===1) checked @endif>
                            </td>
                            <td>{{$page->tags}}</td>
                            <td class="width-16-rem text-left">
                                <a href="{{route('admin.content.page.edit',$page->id)}}" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i> ویرایش</a>
                                <form class="d-inline" method="POST" action="{{route('admin.content.page.destroy',$page->id)}}">
                                    @csrf
                                    {{method_field('delete')}}
